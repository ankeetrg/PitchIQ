...
| AI | Anthropic Claude (primary) / OpenAI (fallback only) |
...
## Tool routing — use the right Claude for the job

| Tool | Best for |
|------|----------|
| **Claude Code** | Editing source files, running typecheck/lint, iterating on components, updating this CLAUDE.md |
...
Claude will flag at the start of a session if the task belongs in a different tool.
